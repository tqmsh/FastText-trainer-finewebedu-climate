...
# Uses src/fasttext/classify_csv.py for counting
# Output: threshold_comparison.json

OUTPUT_FILE="threshold_comparison.json"
NUM_ROWS=500  # Sample 500 rows for testing
DEBUG_FILE="debug_output.json"

# Clear debug file for fresh run
echo "[]" > "$DEBUG_FILE"
echo "Debug output will be written to: $DEBUG_FILE"

# Get dataset sizes (for reference only)
...
import sys
sys.path.insert(0, '.')
from src.fasttext.fasttext_trainer import evaluate_classifier, apply_predict_patch
import fasttext
...
" 2>/dev/null)
    ...
    # Use classify_csv.py to count climate rows (500 sample)
    # Full output goes to debug_output.txt via the Python script; just show summary here
    echo "--- Running classify_csv.py for historical ---"
    hist_full=$(python3 src/fasttext/classify_csv.py -i datasets/historical_regex.csv --mode count --threshold $thresh -n $NUM_ROWS 2>&1)
    # Show results summary (skip chunk debug lines, those go to debug_output.txt)
    echo "$hist_full" | grep -E "(Processing:|Threshold:|Results|Total rows|Climate |Non-climate|DEBUG.*written)"
    hist_result=$(echo "$hist_full" | grep -E "Climate \(" | grep -oP '\d+(?=\s+\()' | head -1)

    echo "--- Running classify_csv.py for modern ---"
    mod_full=$(python3 src/fasttext/classify_csv.py -i datasets/modern_regex.csv --mode count --threshold $thresh -n $NUM_ROWS 2>&1)
    echo "$mod_full" | grep -E "(Processing:|Threshold:|Results|Total rows|Climate |Non-climate|DEBUG.*written)"
    mod_result=$(echo "$mod_full" | grep -E "Climate \(" | grep -oP '\d+(?=\s+\()' | head -1)
    ...
    # Calculate pass rate (based on sample size)
    total_pass=$((hist_result + mod_result))
    total_rows=$((NUM_ROWS * 2))  # Both files have NUM_ROWS sample
...
obj = {
    ...
    'historical_total': $NUM_ROWS,
    'modern_pass': $mod_result,
    'modern_total': $NUM_ROWS,
    'pass_rate': $pass_rate
}
...
data = {
    ...
    'generated': '$(date)',
    'datasets': {
        'historical': {'file': 'datasets/historical_regex.csv', 'sample_size': $NUM_ROWS},
        'modern': {'file': 'datasets/modern_regex.csv', 'sample_size': $NUM_ROWS}
    }
}
with open('$OUTPUT_FILE', 'w') as f:
